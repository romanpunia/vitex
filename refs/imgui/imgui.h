...
// Using dear imgui via a shared refs is not recommended, because of function call overhead and because we don't guarantee backward nor forward ABI compatibility.
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowDemoWindow(bool* p_open = NULL);        // create Demo window (previously called ShowTestWindow). demonstrate most ImGui features. call this to learn about the refs! try to make it always available in your application!
    ...
    // Those three properties are connected. The refs needs to hold their visibility state because it can close popups at any time.
    ...
    IMGUI_API void          SetTabItemClosed(const char* tab_or_docked_window_label);           // notify TabBar or Docking system of a closed tab/window ahead (useful to reduce graphics flicker on reorderable tab bars). For tab-bar: call after BeginTabBar() and before Tab submissions. Otherwise call with a window name.
...
// as this is one of the oldest structure exposed by the refs! Basically, ImDrawList == CmdList)
struct ImDrawData
{
