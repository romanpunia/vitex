...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // When we are too small, start hiding and disabling the grab (this reduce graphics noise on very small window and facilitate using the resize grab)
    float alpha = 1.0f;
...
// Using 'hover_visibility_delay' allows us to hide the highlight and mouse cursor for a short time, which can be convenient to reduce graphics noise.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        // Down the line we should have a cleaner refs-wide concept of Selected vs Active.
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            // NB: '##' is normally used to hide text (as a refs-wide feature), so we need to specify the text range to make sure the ## aren't stripped out here.
