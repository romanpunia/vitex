...
float Weight(in float2 TexCoord)
{
	...
	float Alpha = 1.0 - (Position.z - NearDistance) / NearRange;
...
float4 PS(VertexResult V) : SV_TARGET0
{
	...
	[loop] for (int i = 1; i <= Rings; i++)
	{
		...
		{
			float Step = 6.28318530 / Count;
			float2 Offset = TexCoord + float2(cos(j * Step) * i * Width, sin(j * Step) * i * Height);

			[branch] if (Weight(Offset) >= Amount)
			{
				Color += Aberate(Offset, Amount) * lerp(1.0, (float)i / Rings, Bias) * Circular;
				SampleAmount += lerp(1.0, (float)i / Rings, Bias);
			}
		}
	}
