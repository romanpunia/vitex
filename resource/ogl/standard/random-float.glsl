...
{
	float X = saturate(frac(sin(dot(Value, float2(12.9898f, 78.233f))) * 43758.5453f)) * 2.0f - 1.0f;
	float Y = saturate(frac(sin(dot(Value, float2(12.9898f, 78.233f) * 2.0f)) * 43758.5453f)) * 2.0f - 1.0f;
	return float2(X, Y);
}