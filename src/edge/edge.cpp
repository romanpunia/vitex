...
namespace Edge
{
	...
	bool Initialize(size_t Modules)
	{
		...
		if (Modes & (uint64_t)Init::Network)
		{
			...
			if (WSAStartup(VersionRequested, &WSAData) != 0)
				ED_ERR("[edge] windows socket refs cannot be initialized");
#endif
		}
...
#else
			ED_WARN("[edge] openssl ssl cannot be initialized");
#endif
		}
		...
		if (Modes & (uint64_t)Init::SDL2)
		{
			...
			if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_GAMECONTROLLER | SDL_INIT_HAPTIC) == 0)
			{
				...
				ED_ERR("[edge] %s", SDL_GetError());
#else
			ED_WARN("[edge] sdl2 cannot be initialized");
#endif
		}
		...
		if (Modes & (uint64_t)Init::Locale)
		{
			if (!setlocale(LC_TIME, "C"))
				ED_WARN("[edge] en-US locale cannot be initialized");
		}
	...
	bool Uninitialize()
	{
		...
		if (Modes & (uint64_t)Init::SSL)
		{
			...
			{
				ED_DELETE(vector, CryptoLocks);
				CryptoLocks = nullptr;
			}
#else
			ED_WARN("[edge] openssl ssl cannot be uninitialized");
#endif
		}
		...
		if (Modes & (uint64_t)Init::SDL2)
		{
			...
			SDL_Quit();
#else
			ED_WARN("[edge] sdl2 cannot be uninitialized");
#endif
		}
