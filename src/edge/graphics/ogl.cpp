...
namespace
{
	...
	void OGL_CopyTextureFace2D_3_0(Edge::Compute::CubeFace Face, GLuint SrcName, GLuint DestName, GLint Width, GLint Height)
	{
		GLint LastFrameBuffer = 0;
		glGetIntegerv(GL_FRAMEBUFFER_BINDING, &LastFrameBuffer);
	...
	namespace Graphics
	{
		namespace OGL
		{
			...
			OGLCubemap::OGLCubemap(const Desc& I) : Cubemap(I)
			{
				...
				ED_ASSERT_V(Target != nullptr && Target->Resource != GL_NONE, "render target should be valid");
				ED_ASSERT_V(!((OGLFrameBuffer*)I.Source->GetTargetBuffer())->Backbuffer, "cannot copy from backbuffer directly");
			...
			{
				ED_ASSERT_V(State != nullptr, "blend state should be set");
			...
			{
				ED_ASSERT_V(State != nullptr, "rasterizer state should be set");
			...
			{
				ED_ASSERT_V(State != nullptr, "depth stencil state should be set");
			...
			void OGLDevice::SetShader(Shader* Resource, unsigned int Type)
			{
				...
				if (StatusCode != GL_TRUE)
				{
					...
					char* Buffer = ED_MALLOC(char, sizeof(char) * (Size + 1));
					glGetProgramInfoLog(Program, Size, &Size, Buffer);
					ED_ERR("[ogl] couldn't link shaders\n\t%.*s", Size, Buffer);
					ED_FREE(Buffer);
			...
			void OGLDevice::SetWriteable(Texture2D** Resource, unsigned int Slot, unsigned int Count, bool Computable)
			{
				...
				for (unsigned int i = 0; i < Count; i++)
				{
					...
					ED_ASSERT_V(!IResource || !IResource->Backbuffer, "resource 2d #%i should not be back buffer texture", (int)i);
			...
			void OGLDevice::SetTarget(Graphics::RenderTarget* Resource, unsigned int Target, float R, float G, float B)
			{
				...
				ED_ASSERT_V(TargetBuffer != nullptr, "target buffer should be set");
			...
			{
				ED_ASSERT_V(Count > 0 && Value != nullptr, "at least one viewport should be set");
			...
			{
				ED_ASSERT_V(Count > 0 && Value != nullptr, "at least one scissor rect should be set");
			...
			{
				ED_ASSERT_V(Resource != nullptr, "resource should be set");
				ED_ASSERT_V(!((OGLTexture2D*)Resource)->Backbuffer, "resource 2d should not be back buffer texture");
			...
			bool OGLDevice::CopyTexture2D(Texture2D* Resource, Texture2D** Result)
			{
				...
				ED_ASSERT(!*Result || !((OGLTexture2D*)(*Result))->Backbuffer, false, "output resource 2d should not be back buffer");

				OGLTexture2D* IResource = (OGLTexture2D*)Resource;
				if (IResource->Backbuffer)
					return CopyBackBuffer(Result);

				ED_ASSERT(IResource->Resource != GL_NONE, false, "resource should be valid");
			...
			bool OGLDevice::CopyTexture2D(Graphics::RenderTarget* Resource, unsigned int Target, Texture2D** Result)
			{
				...
				ED_ASSERT(TargetBuffer != nullptr, false, "target buffer should be set");
				if (TargetBuffer->Backbuffer)
				{
					if (!*Result)
					{
			...
			bool OGLDevice::CopyTexture2D(MultiRenderTargetCube* Resource, unsigned int Cube, Compute::CubeFace Face, Texture2D** Result)
			{
				...
				ED_ASSERT(Cube < (uint32_t)IResource->Target, false, "cube index should be less than %i", (int)IResource->Target);
				int LastTexture = GL_NONE, Width, Height;
				glGetIntegerv(GL_TEXTURE_BINDING_CUBE_MAP, &LastTexture);
			...
			bool OGLDevice::CopyTarget(Graphics::RenderTarget* From, unsigned int FromTarget, Graphics::RenderTarget* To, unsigned int ToTarget)
			{
				...
				ED_ASSERT(Source != GL_NONE, false, "from should be set");
				ED_ASSERT(Dest != GL_NONE, false, "to should be set");
			...
			void OGLDevice::GenerateMips(Texture2D* Resource)
			{
				...
				ED_ASSERT_V(!IResource->Backbuffer, "resource 2d should not be back buffer texture");
				ED_ASSERT_V(IResource->Resource != GL_NONE, "resource should be valid");
			...
			Shader* OGLDevice::CreateShader(const Shader::Desc& I)
			{
				...
				if (!Preprocess(F))
				{
					ED_ERR("[ogl] shader preprocessing failed");
					return Result;
				}
				...
				if (F.Data.find(VertexEntry) != std::string::npos)
				{
					...
					if (!GetProgramCache(Stage, &Bytecode))
					{
						...
						{
							ED_ERR("[ogl] vertex shader transpiling failed");
							return Result;
						}
						...
						if (!SetProgramCache(Stage, Bytecode))
							ED_WARN("[ogl] couldn't cache vertex shader");
					}
					else
					{
						Data = Bytecode.c_str();
					...
					ED_DEBUG("[ogl] compile %s vertex shader bytecode", Stage.c_str());
					...
					if (State == GL_FALSE)
					{
						...
						ED_ERR("[ogl] couldn't compile vertex shader\n\t%.*s", Size, Buffer);
						ED_FREE(Buffer);
					}
				}
				...
				if (F.Data.find(PixelEntry) != std::string::npos)
				{
					...
					if (!GetProgramCache(Stage, &Bytecode))
					{
						...
						{
							ED_ERR("[ogl] pixel shader transpiling failed");
							return Result;
						}
						...
						Size = (GLint)Bytecode.size();
						if (!SetProgramCache(Stage, Bytecode))
							ED_WARN("[ogl] couldn't cache pixel shader");
					}
					else
					{
						Data = Bytecode.c_str();
					...
					ED_DEBUG("[ogl] compile %s pixel shader bytecode", Stage.c_str());
					...
					if (State == GL_FALSE)
					{
						...
						ED_ERR("[ogl] couldn't compile pixel shader\n\t%.*s", Size, Buffer);
						ED_FREE(Buffer);
					}
				}
				...
				if (F.Data.find(GeometryEntry) != std::string::npos)
				{
					...
					if (!GetProgramCache(Stage, &Bytecode))
					{
						...
						{
							ED_ERR("[ogl] geometry shader transpiling failed");
							return Result;
						}
						...
						Size = (GLint)Bytecode.size();
						if (!SetProgramCache(Stage, Bytecode))
							ED_WARN("[ogl] couldn't cache geometry shader");
					}
					else
					{
						Data = Bytecode.c_str();
					...
					ED_DEBUG("[ogl] compile %s geometry shader bytecode", Stage.c_str());
					...
					if (State == GL_FALSE)
					{
						...
						ED_ERR("[ogl] couldn't compile geometry shader\n\t%.*s", Size, Buffer);
						ED_FREE(Buffer);
					}
				}
				...
				if (F.Data.find(ComputeEntry) != std::string::npos)
				{
					...
					if (!GetProgramCache(Stage, &Bytecode))
					{
						...
						{
							ED_ERR("[ogl] compute shader transpiling failed");
							return Result;
						}
						...
						Size = (GLint)Bytecode.size();
						if (!SetProgramCache(Stage, Bytecode))
							ED_WARN("[ogl] couldn't cache compute shader");
					}
					else
					{
						Data = Bytecode.c_str();
					...
					ED_DEBUG("[ogl] compile %s compute shader bytecode", Stage.c_str());
					...
					if (State == GL_FALSE)
					{
						...
						ED_ERR("[ogl] couldn't compile compute shader\n\t%.*s", Size, Buffer);
						ED_FREE(Buffer);
					}
				}
				...
				if (F.Data.find(HullEntry) != std::string::npos)
				{
					...
					if (!GetProgramCache(Stage, &Bytecode))
					{
						...
						{
							ED_ERR("[ogl] hull shader transpiling failed");
							return Result;
						}
						...
						Size = (GLint)Bytecode.size();
						if (!SetProgramCache(Stage, Bytecode))
							ED_WARN("[ogl] couldn't cache hull shader");
					}
					else
					{
						Data = Bytecode.c_str();
					...
					ED_DEBUG("[ogl] compile %s hull shader bytecode", Stage.c_str());
					...
					if (State == GL_FALSE)
					{
						...
						ED_ERR("[ogl] couldn't compile hull shader\n\t%.*s", Size, Buffer);
						ED_FREE(Buffer);
					}
				}
				...
				if (F.Data.find(DomainEntry) != std::string::npos)
				{
					...
					if (!GetProgramCache(Stage, &Bytecode))
					{
						...
						{
							ED_ERR("[ogl] domain shader transpiling failed");
							return Result;
						}
						...
						Size = (GLint)Bytecode.size();
						if (!SetProgramCache(Stage, Bytecode))
							ED_WARN("[ogl] couldn't cache domain shader");
					}
					else
					{
						Data = Bytecode.c_str();
					...
					ED_DEBUG("[ogl] compile %s domain shader bytecode", Stage.c_str());
					...
					if (State == GL_FALSE)
					{
						...
						ED_ERR("[ogl] couldn't compile domain shader\n\t%.*s", Size, Buffer);
						ED_FREE(Buffer);
					}
				}
			...
			TextureCube* OGLDevice::CreateTextureCube(Texture2D* Resource[6])
			{
				...
				for (unsigned int i = 0; i < 6; i++)
				{
					ED_ASSERT(Resource[i] != nullptr, nullptr, "face #%i should be set", (int)i);
					ED_ASSERT(!((OGLTexture2D*)Resource[i])->Backbuffer, nullptr, "resource 2d should not be back buffer texture");
			...
			TextureCube* OGLDevice::CreateTextureCube(Texture2D* Resource)
			{
				...
				ED_ASSERT(!((OGLTexture2D*)Resource)->Backbuffer, nullptr, "resource 2d should not be back buffer texture");
				...
				{
					ED_ERR("[ogl] couldn't create texture cube because width or height cannot be not divided");
					return nullptr;
				}
				...
				GLubyte* Pixels = ED_MALLOC(GLubyte, Size);
				Result->Format = SizeFormat;
			...
			{
				ED_ASSERT(Basis[0] && Basis[1] && Basis[2] && Basis[3] && Basis[4] && Basis[5], nullptr, "all 6 faces should be set");
				...
				GLint SizeFormat = OGLDevice::GetSizedFormat(Result->FormatMode);
				GLint BaseFormat = OGLDevice::GetBaseFormat(Result->FormatMode);
				GLubyte* Pixels = ED_MALLOC(GLubyte, sizeof(GLubyte) * Resources[0]->Width * Resources[0]->Height);
				Result->Format = SizeFormat;
			...
			bool OGLDevice::CreateDirectBuffer(size_t Size)
			{
				...
				if (Immediate.VertexShader == GL_NONE)
				{
					...
					if (StatusCode == GL_FALSE)
					{
						...
						char* Buffer = ED_MALLOC(char, sizeof(char) * (BufferSize + 1));
			...
			{
				ED_ASSERT_V(Data != nullptr, "buffer should not be empty");
				if (!Size)
					return;
			...
			GLenum OGLDevice::GetAccessControl(CPUAccess Access, ResourceUsage Usage)
			{
				switch (Usage)
				{
					case Edge::Graphics::ResourceUsage::Default:
						switch (Access)
						{
					...
					case Edge::Graphics::ResourceUsage::Immutable:
						return GL_STATIC_DRAW;
					case Edge::Graphics::ResourceUsage::Dynamic:
						switch (Access)
						{
							case Edge::Graphics::CPUAccess::Read:
					...
					case Edge::Graphics::ResourceUsage::Staging:
						return GL_DYNAMIC_READ;
					default:
						return GL_STATIC_DRAW;
				}
			...
			{
				ED_ASSERT_V(X && Y && Z && W, "xyzw should be set");
				switch (Value)
				{
					case Edge::Graphics::Format::A8_Unorm:
						*X = *Y = *Z = 0;
						*W = 8;
						break;
					...
					case Edge::Graphics::Format::D32_Float:
						...
						*X = 32;
						break;
					case Edge::Graphics::Format::R10G10B10A2_Uint:
					case Edge::Graphics::Format::R10G10B10A2_Unorm:
						*W = 2;
						*Z = 10;
						*X = *Y = 11;
						break;
					case Edge::Graphics::Format::R11G11B10_Float:
						*W = 0;
						*Z = 10;
						*X = *Y = 11;
						break;
					case Edge::Graphics::Format::R16G16B16A16_Float:
					case Edge::Graphics::Format::R16G16B16A16_Sint:
					case Edge::Graphics::Format::R16G16B16A16_Snorm:
					case Edge::Graphics::Format::R16G16B16A16_Uint:
					case Edge::Graphics::Format::R16G16B16A16_Unorm:
						*X = *Y = *Z = *W = 16;
						break;
					case Edge::Graphics::Format::R16G16_Float:
					case Edge::Graphics::Format::R16G16_Sint:
					case Edge::Graphics::Format::R16G16_Snorm:
					case Edge::Graphics::Format::R16G16_Uint:
					case Edge::Graphics::Format::R16G16_Unorm:
						*Z = *W = 0;
						*X = *Y = 16;
						break;
					case Edge::Graphics::Format::R16_Float:
					case Edge::Graphics::Format::R16_Sint:
					case Edge::Graphics::Format::R16_Snorm:
					case Edge::Graphics::Format::R16_Uint:
					case Edge::Graphics::Format::R16_Unorm:
					case Edge::Graphics::Format::D16_Unorm:
						*Y = *Z = *W = 0;
						*X = 16;
						break;
					case Edge::Graphics::Format::R1_Unorm:
						*Y = *Z = *W = 0;
						*X = 1;
						break;
					case Edge::Graphics::Format::R32G32B32A32_Float:
					case Edge::Graphics::Format::R32G32B32A32_Sint:
					case Edge::Graphics::Format::R32G32B32A32_Uint:
						*X = *Y = *Z = *W = 32;
						break;
					case Edge::Graphics::Format::R32G32B32_Float:
					case Edge::Graphics::Format::R32G32B32_Sint:
					case Edge::Graphics::Format::R32G32B32_Uint:
						*W = 0;
						*X = *Y = *Z = 32;
						break;
					case Edge::Graphics::Format::R32G32_Float:
					case Edge::Graphics::Format::R32G32_Sint:
					case Edge::Graphics::Format::R32G32_Uint:
						*Z = *W = 0;
						*X = *Y = 32;
						break;
					case Edge::Graphics::Format::R32_Float:
					case Edge::Graphics::Format::R32_Sint:
					case Edge::Graphics::Format::R32_Uint:
						*Y = *Z = *W = 0;
						*X = 32;
						break;
					case Edge::Graphics::Format::R8G8_Sint:
					case Edge::Graphics::Format::R8G8_Snorm:
					case Edge::Graphics::Format::R8G8_Uint:
					case Edge::Graphics::Format::R8G8_Unorm:
						*Z = *W = 0;
						*X = *Y = 8;
						break;
					case Edge::Graphics::Format::R8_Sint:
					case Edge::Graphics::Format::R8_Snorm:
					case Edge::Graphics::Format::R8_Uint:
					case Edge::Graphics::Format::R8_Unorm:
						*Y = *Z = *W = 0;
						*X = 8;
						break;
					case Edge::Graphics::Format::R9G9B9E5_Share_Dexp:
						break;
					case Edge::Graphics::Format::R8G8B8A8_Sint:
					case Edge::Graphics::Format::R8G8B8A8_Snorm:
					case Edge::Graphics::Format::R8G8B8A8_Uint:
					case Edge::Graphics::Format::R8G8B8A8_Unorm:
					case Edge::Graphics::Format::R8G8B8A8_Unorm_SRGB:
					case Edge::Graphics::Format::R8G8_B8G8_Unorm:
					default:
						*X = *Y = *Z = *W = 8;
						break;
				}
			...
			void OGLDevice::DebugMessage(GLenum Source, GLenum Type, GLuint Id, GLenum Severity, GLsizei Length, const GLchar* Message, const void* Data)
			{
				...
				switch (Severity)
				{
					case GL_DEBUG_SEVERITY_HIGH:
						ED_ERR("[ogl] %s (%s:%d): %s", _Source, _Type, Id, Message);
						break;
					case GL_DEBUG_SEVERITY_MEDIUM:
						ED_WARN("[ogl] %s (%s:%d): %s", _Source, _Type, Id, Message);
						break;
					case GL_DEBUG_SEVERITY_LOW:
						ED_DEBUG("[ogl] %s (%s:%d): %s", _Source, _Type, Id, Message);
						break;
				}
			}
		}
