	...
	namespace Network
	{
		namespace PDB
		{
			...
			public:
				...
				{
					ED_ASSERT(Callback, false, "callback should be set");
					if (Source[0] == '[')
					{
						while (Position < Size)
						{
			...
			static void PQlogMessage(TConnection* Base)
			{
				...
				ED_ERR("[pqerr] %s", Errors.size() > 1 ? Result.c_str() : Result.c_str() + 2);
			}
			static void PQlogNotice(void*, const char* Message)
			{
				...
				ED_WARN("[pqnotice] %s", Errors.size() > 1 ? Result.c_str() : Result.c_str() + 2);
			}
			...
			Address::Address(const std::string& URI)
			{
				...
				else
				{
					if (Message != nullptr)
					{
						ED_ERR("[pq] couldn't parse URI string\n\t%s", Message);
						PQfreemem(Message);
					}
				}
#endif
			...
			{
				const char** Result = ED_MALLOC(const char*, sizeof(const char*) * (Params.size() + 1));
				size_t Index = 0;
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Data != nullptr, -1, "data should be set");
				ED_ASSERT(Base != nullptr, -1, "context should be valid");
				ED_ASSERT(RowIndex != std::numeric_limits<size_t>::max(), -1, "row should be valid");
				ED_ASSERT(ColumnIndex != std::numeric_limits<size_t>::max(), -1, "column should be valid");
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Base != nullptr, nullptr, "context should be valid");
				ED_ASSERT(RowIndex != std::numeric_limits<size_t>::max(), nullptr, "row should be valid");
				ED_ASSERT(ColumnIndex != std::numeric_limits<size_t>::max(), nullptr, "column should be valid");
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Base != nullptr, 0, "context should be valid");
				ED_ASSERT(RowIndex != std::numeric_limits<size_t>::max(), 0, "row should be valid");
				ED_ASSERT(ColumnIndex != std::numeric_limits<size_t>::max(), 0, "column should be valid");
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Base != nullptr, InvalidOid, "context should be valid");
				ED_ASSERT(RowIndex != std::numeric_limits<size_t>::max(), InvalidOid, "row should be valid");
				ED_ASSERT(ColumnIndex != std::numeric_limits<size_t>::max(), InvalidOid, "column should be valid");
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Base != nullptr, Row(nullptr, std::numeric_limits<size_t>::max()), "context should be valid");
				ED_ASSERT(RowIndex != std::numeric_limits<size_t>::max(), Row(nullptr, std::numeric_limits<size_t>::max()), "row should be valid");
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Name != nullptr, Column(nullptr, std::numeric_limits<size_t>::max(), std::numeric_limits<size_t>::max()), "name should be set");
				if (!Base || RowIndex == std::numeric_limits<size_t>::max())
					return Column(Base, std::numeric_limits<size_t>::max(), std::numeric_limits<size_t>::max());
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Output != nullptr && Size > 0, false, "output should be valid");
				ED_ASSERT(Base != nullptr, false, "context should be valid");
				ED_ASSERT(RowIndex != std::numeric_limits<size_t>::max(), false, "row should be valid");
...
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Base != nullptr, -1, "context should be valid");
				ED_ASSERT(!Name.empty(), -1, "name should not be empty");
			...
			{
#ifdef ED_HAS_POSTGRESQL
				ED_ASSERT(Base != nullptr, QueryExec::Empty_Query, "context should be valid");
			...
			{
				ED_ASSERT(!Base.empty(), Base.front(), "index outside of range");
				return Base.front();
			}
			...
			{
				ED_ASSERT(Index < Base.size(), Base.front(), "index outside of range");
				return Base[Index];
			}
			...
			void Cluster::SetCacheDuration(QueryOp CacheId, uint64_t Duration)
			{
				switch (CacheId)
				{
					...
					case QueryOp::CacheLong:
						...
						break;
					default:
						ED_ASSERT_V(false, "cache id should be valid cache category");
						break;
				}
			}
			...
			void Cluster::TryUnassign(Connection* Base, Request* Context)
			{
				...
				ED_DEBUG("[pq] end transaction on 0x%" PRIXPTR, (uintptr_t)Base);
			...
			bool Cluster::Reestablish(Connection* Target)
			{
				...
				ED_DEBUG("[pq] try reconnect on 0x%" PRIXPTR, (uintptr_t)Target->Base);
			...
			bool Cluster::TryAssign(Connection* Base, Request* Context)
			{
				...
				ED_DEBUG("[pq] start transaction on 0x%" PRIXPTR, (uintptr_t)Base);
				Base->InSession = true;
				return true;
			}
