...
namespace Edge
{
	namespace Network
	{
		namespace SMTP
		{
			...
			{
				ED_ASSERT(Keyword != nullptr, false, "keyword should be set");
			...
			bool Client::Authorize(const ReplyCallback& Callback)
			{
				...
				else if (CanRequest("CRAM-MD5"))
				{
					return SendRequest(334, "AUTH CRAM-MD5\r\n", [this, Callback]()
					{
						...
						{
							ED_FREE(UserChallenge);
							ED_FREE(UserPassword);
			...
			unsigned char* Client::Unicode(const char* String)
			{
				...
				auto* Output = ED_MALLOC(unsigned char, sizeof(unsigned char) * (Length + 1));
