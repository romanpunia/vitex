...
float SampleCascade(in float3 Position, uniform uint Index)
{
...
float4 PS(VertexResult V) : SV_TARGET0
{
    ...
    float3 H = Mat.Emission.xyz * GetThickness(P, Position, Frag.Normal, Mat.Scatter);
    E = Lighting * (Frag.Diffuse * (E + H) + O);

    float S = SampleCascade(Frag.Position, 0);
    [branch] if (S >= 0.0)
        return float4(E * (S + H), 1.0);
        
    S = SampleCascade(Frag.Position, 1);
    [branch] if (S >= 0.0)
        return float4(E * (S + H), 1.0);
        
    S = SampleCascade(Frag.Position, 2);
    [branch] if (S >= 0.0)
        return float4(E * (S + H), 1.0);
        
    S = SampleCascade(Frag.Position, 3);
    [branch] if (S >= 0.0)
        return float4(E * (S + H), 1.0);
        
    S = SampleCascade(Frag.Position, 4);
    [branch] if (S >= 0.0)
        return float4(E * (S + H), 1.0);
        
    S = SampleCascade(Frag.Position, 5);
    [branch] if (S >= 0.0)
        return float4(E * (S + H), 1.0);
