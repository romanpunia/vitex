...
void VMCArray::SetMemoryFunctions(asALLOCFUNC_t allocFunc, asFREEFUNC_t freeFunc)
{
	userAlloc = allocFunc;
	userFree = freeFunc;
}
...
VMCArray* VMCArray::Create(asITypeInfo *ti, asUINT length)
{
	// Allocate the memory
	void *mem = userAlloc(sizeof(VMCArray));
	if (mem == 0)
	{
...
VMCArray* VMCArray::Create(asITypeInfo *ti, void *initList)
{
	// Allocate the memory
	void *mem = userAlloc(sizeof(VMCArray));
	if (mem == 0)
...
VMCArray* VMCArray::Create(asITypeInfo *ti, asUINT length, void *defVal)
{
	// Allocate the memory
	void *mem = userAlloc(sizeof(VMCArray));
	if (mem == 0)
...
VMCArray* VMCArray::Create(asITypeInfo *ti)
{
	return VMCArray::Create(ti, asUINT(0));
}
...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in)", asFUNCTIONPR(VMCArray::Create, (asITypeInfo*), VMCArray*), asCALL_CDECL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint length) explicit", asFUNCTIONPR(VMCArray::Create, (asITypeInfo*, asUINT), VMCArray*), asCALL_CDECL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint length, const T &in value)", asFUNCTIONPR(VMCArray::Create, (asITypeInfo*, asUINT, void *), VMCArray*), asCALL_CDECL); assert(r >= 0);

	// Register the factory that will be used for initialization lists
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_LIST_FACTORY, "array<T>@ f(int&in type, int&in list) {repeat T}", asFUNCTIONPR(VMCArray::Create, (asITypeInfo*, void*), VMCArray*), asCALL_CDECL); assert(r >= 0);

	// The memory management methods
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_ADDREF, "void f()", asMETHOD(VMCArray, AddRef), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_RELEASE, "void f()", asMETHOD(VMCArray, Release), asCALL_THISCALL); assert(r >= 0);

	// The index operator returns the template subtype
	r = engine->RegisterObjectMethod("array<T>", "T &opIndex(uint index)", asMETHODPR(VMCArray, At, (asUINT), void*), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "const T &opIndex(uint index) const", asMETHODPR(VMCArray, At, (asUINT) const, const void*), asCALL_THISCALL); assert(r >= 0);

	// The assignment operator
	r = engine->RegisterObjectMethod("array<T>", "array<T> &opAssign(const array<T>&in)", asMETHOD(VMCArray, operator=), asCALL_THISCALL); assert(r >= 0);

	// Other methods
	r = engine->RegisterObjectMethod("array<T>", "void insertAt(uint index, const T&in value)", asMETHODPR(VMCArray, InsertAt, (asUINT, void *), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void insertAt(uint index, const array<T>& arr)", asMETHODPR(VMCArray, InsertAt, (asUINT, const VMCArray &), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void insertLast(const T&in value)", asMETHOD(VMCArray, InsertLast), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void removeAt(uint index)", asMETHOD(VMCArray, RemoveAt), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void removeLast()", asMETHOD(VMCArray, RemoveLast), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void removeRange(uint start, uint count)", asMETHOD(VMCArray, RemoveRange), asCALL_THISCALL); assert(r >= 0);
	...
	r = engine->RegisterObjectMethod("array<T>", "uint length() const", asMETHOD(VMCArray, GetSize), asCALL_THISCALL); assert(r >= 0);
#endif
	r = engine->RegisterObjectMethod("array<T>", "void reserve(uint length)", asMETHOD(VMCArray, Reserve), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void resize(uint length)", asMETHODPR(VMCArray, Resize, (asUINT), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void sortAsc()", asMETHODPR(VMCArray, SortAsc, (), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void sortAsc(uint startAt, uint count)", asMETHODPR(VMCArray, SortAsc, (asUINT, asUINT), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void sortDesc()", asMETHODPR(VMCArray, SortDesc, (), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void sortDesc(uint startAt, uint count)", asMETHODPR(VMCArray, SortDesc, (asUINT, asUINT), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void reverse()", asMETHOD(VMCArray, Reverse), asCALL_THISCALL); assert(r >= 0);
	...
	r = engine->RegisterObjectMethod("array<T>", "int find(const T&in if_handle_then_const value) const", asMETHODPR(VMCArray, Find, (void*) const, int), asCALL_THISCALL); assert(r >= 0);
	// TODO: It should be "int find(const T&in value, uint startAt = 0) const"
	r = engine->RegisterObjectMethod("array<T>", "int find(uint startAt, const T&in if_handle_then_const value) const", asMETHODPR(VMCArray, Find, (asUINT, void*) const, int), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "int findByRef(const T&in if_handle_then_const value) const", asMETHODPR(VMCArray, FindByRef, (void*) const, int), asCALL_THISCALL); assert(r >= 0);
	// TODO: It should be "int findByRef(const T&in value, uint startAt = 0) const"
	r = engine->RegisterObjectMethod("array<T>", "int findByRef(uint startAt, const T&in if_handle_then_const value) const", asMETHODPR(VMCArray, FindByRef, (asUINT, void*) const, int), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "bool opEquals(const array<T>&in) const", asMETHOD(VMCArray, operator==), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "bool isEmpty() const", asMETHOD(VMCArray, IsEmpty), asCALL_THISCALL); assert(r >= 0);
	...
	r = engine->RegisterObjectMethod("array<T>", "void sort(const less &in, uint startAt = 0, uint count = uint(-1))", asMETHODPR(VMCArray, Sort, (asIScriptFunction*, asUINT, asUINT), void), asCALL_THISCALL); assert(r >= 0);

#if AS_USE_STLNAMES != 1 && AS_USE_ACCESSORS == 1
	// Register virtual properties
	r = engine->RegisterObjectMethod("array<T>", "uint get_length() const property", asMETHOD(VMCArray, GetSize), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void set_length(uint) property", asMETHODPR(VMCArray, Resize, (asUINT), void), asCALL_THISCALL); assert(r >= 0);
#endif

	// Register GC behaviours in case the array needs to be garbage collected
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_GETREFCOUNT, "int f()", asMETHOD(VMCArray, GetRefCount), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_SETGCFLAG, "void f()", asMETHOD(VMCArray, SetFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_GETGCFLAG, "bool f()", asMETHOD(VMCArray, GetFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(VMCArray, EnumReferences), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(VMCArray, ReleaseAllHandles), asCALL_THISCALL); assert(r >= 0);

#if AS_USE_STLNAMES == 1
	// Same as length
	r = engine->RegisterObjectMethod("array<T>", "uint size() const", asMETHOD(VMCArray, GetSize), asCALL_THISCALL); assert(r >= 0);
	// Same as isEmpty
	r = engine->RegisterObjectMethod("array<T>", "bool empty() const", asMETHOD(VMCArray, IsEmpty), asCALL_THISCALL); assert(r >= 0);
	// Same as insertLast
	r = engine->RegisterObjectMethod("array<T>", "void push_back(const T&in)", asMETHOD(VMCArray, InsertLast), asCALL_THISCALL); assert(r >= 0);
	// Same as removeLast
	r = engine->RegisterObjectMethod("array<T>", "void pop_back()", asMETHOD(VMCArray, RemoveLast), asCALL_THISCALL); assert(r >= 0);
	// Same as insertAt
	r = engine->RegisterObjectMethod("array<T>", "void insert(uint index, const T&in value)", asMETHODPR(VMCArray, InsertAt, (asUINT, void *), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "void insert(uint index, const array<T>& arr)", asMETHODPR(VMCArray, InsertAt, (asUINT, const VMCArray &), void), asCALL_THISCALL); assert(r >= 0);
	// Same as removeAt
	r = engine->RegisterObjectMethod("array<T>", "void erase(uint)", asMETHOD(VMCArray, RemoveAt), asCALL_THISCALL); assert(r >= 0);
#endif
}
...
VMCArray::VMCArray(asITypeInfo *ti, void *buf)
{
...
VMCArray::VMCArray(asUINT length, asITypeInfo *ti)
{
	// The object type should be the template instance of the array
	assert(ti && string(ti->GetName()) == "array");
...
VMCArray::VMCArray(asUINT length, void *defVal, asITypeInfo *ti)
{
	// The object type should be the template instance of the array
	assert(ti && string(ti->GetName()) == "array");
...
void VMCArray::SetValue(asUINT index, void *value)
{
...
void VMCArray::Reserve(asUINT maxElements)
{
...
void VMCArray::Resize(asUINT numElements)
{
...
void VMCArray::RemoveRange(asUINT start, asUINT count)
{
	if (count == 0)
		return;
...
void VMCArray::Resize(int delta, asUINT at)
{
	if (delta < 0)
	{
...
bool VMCArray::CheckMaxSize(asUINT numElements)
{
...
asITypeInfo *VMCArray::GetArrayObjectType() const
{
	return objType;
}
...
void VMCArray::InsertAt(asUINT index, void *value)
{
...
void VMCArray::InsertAt(asUINT index, const VMCArray &arr)
{
	if (index > buffer->numElements)
	{
		asIScriptContext *ctx = asGetActiveContext();
...
void VMCArray::RemoveAt(asUINT index)
{
	if (index >= buffer->numElements)
	{
		// If this is called from a script we raise a script exception
...
const void *VMCArray::At(asUINT index) const
{
...
void VMCArray::CreateBuffer(SArrayBuffer **buf, asUINT numElements)
{
...
void VMCArray::DeleteBuffer(SArrayBuffer *buf)
{
...
void VMCArray::Construct(SArrayBuffer *buf, asUINT start, asUINT end)
{
	if ((subTypeId & asTYPEID_MASK_OBJECT) && !(subTypeId & asTYPEID_OBJHANDLE))
	{
...
void VMCArray::Destruct(SArrayBuffer *buf, asUINT start, asUINT end)
{
	if (subTypeId & asTYPEID_MASK_OBJECT)
	{
		asIScriptEngine *engine = objType->GetEngine();
...
bool VMCArray::Less(const void *a, const void *b, bool asc, asIScriptContext *ctx, SArrayCache *cache)
{
	if (!asc)
	{
		// Swap items
...
bool VMCArray::Equals(const void *a, const void *b, asIScriptContext *ctx, SArrayCache *cache) const
{
	if (!(subTypeId & ~asTYPEID_MASK_SEQNBR))
	{
		// Simple compare of values
...
int VMCArray::FindByRef(asUINT startAt, void *ref) const
{
...
int VMCArray::Find(asUINT startAt, void *value) const
{
...
void VMCArray::SortAsc(asUINT startAt, asUINT count)
{
...
void VMCArray::SortDesc(asUINT startAt, asUINT count)
{
	Sort(startAt, count, false);
}
void VMCArray::Sort(asUINT startAt, asUINT count, bool asc)
{
...
void VMCArray::Sort(asIScriptFunction *func, asUINT startAt, asUINT count)
{
	// No need to sort
	if (count < 2)
		return;
...
void VMCArray::CopyBuffer(SArrayBuffer *dst, SArrayBuffer *src)
{
	asIScriptEngine *engine = objType->GetEngine();
	if (subTypeId & asTYPEID_OBJHANDLE)
	{
...
void VMCArray::EnumReferences(asIScriptEngine *engine)
{
...
void VMCArray::ReleaseAllHandles(asIScriptEngine *)
{
...
void VMCArray::Release() const
{
	...
	if (asAtomicDec(refCount) == 0)
	{
		...
		this->~VMCArray();
		userFree(const_cast<VMCArray*>(this));
	}
}
...
static void ScriptArrayFactory_Generic(asIScriptGeneric *gen)
{
	...
	*reinterpret_cast<VMCArray**>(gen->GetAddressOfReturnLocation()) = VMCArray::Create(ti);
}
static void ScriptArrayFactory2_Generic(asIScriptGeneric *gen)
{
	...
	*reinterpret_cast<VMCArray**>(gen->GetAddressOfReturnLocation()) = VMCArray::Create(ti, length);
}
static void ScriptArrayListFactory_Generic(asIScriptGeneric *gen)
{
	...
	*reinterpret_cast<VMCArray**>(gen->GetAddressOfReturnLocation()) = VMCArray::Create(ti, buf);
}
static void ScriptArrayFactoryDefVal_Generic(asIScriptGeneric *gen)
{
	...
	*reinterpret_cast<VMCArray**>(gen->GetAddressOfReturnLocation()) = VMCArray::Create(ti, length, defVal);
}
...
{
	VMCArray *other = (VMCArray*)gen->GetArgObject(0);
	VMCArray *self = (VMCArray*)gen->GetObject();
	*self = *other;
...
{
	asUINT index = gen->GetArgDWord(0);
	VMCArray *array = (VMCArray*)gen->GetArgAddress(1);
	VMCArray *self = (VMCArray*)gen->GetObject();
