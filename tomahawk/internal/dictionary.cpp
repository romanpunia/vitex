...
VMCDictionary *VMCDictionary::Create(asIScriptEngine *engine)
{
	// Use the custom memory routine from AngelScript to allow application to better control how much memory is used
	VMCDictionary *obj = (VMCDictionary*)asAllocMem(sizeof(VMCDictionary));
	new(obj) VMCDictionary(engine);
	return obj;
}
VMCDictionary *VMCDictionary::Create(asBYTE *buffer)
{
	// Use the custom memory routine from AngelScript to allow application to better control how much memory is used
	VMCDictionary *obj = (VMCDictionary*)asAllocMem(sizeof(VMCDictionary));
	new(obj) VMCDictionary(buffer);
	return obj;
}
VMCDictionary::VMCDictionary(asIScriptEngine *engine)
{
	Init(engine);
}
void VMCDictionary::Init(asIScriptEngine *e)
{
...
VMCDictionary::VMCDictionary(asBYTE *buffer)
{
...
VMCDictionary::~VMCDictionary()
{
	// Delete all keys and values
	DeleteAll();
}
void VMCDictionary::AddRef() const
{
	// We need to clear the GC flag
	gcFlag = false;
	asAtomicInc(refCount);
}
void VMCDictionary::Release() const
{
	// We need to clear the GC flag
	gcFlag = false;
	...
	{
		this->~VMCDictionary();
		asFreeMem(const_cast<VMCDictionary*>(this));
	}
}
int VMCDictionary::GetRefCount()
{
	return refCount;
}
void VMCDictionary::SetGCFlag()
{
	gcFlag = true;
}
bool VMCDictionary::GetGCFlag()
{
	return gcFlag;
}
void VMCDictionary::EnumReferences(asIScriptEngine *inEngine)
{
...
void VMCDictionary::ReleaseAllReferences(asIScriptEngine*)
{
	// We're being told to release all references in 
	// order to break circular references for dead objects
	DeleteAll();
}
VMCDictionary &VMCDictionary::operator =(const VMCDictionary &other)
{
...
VMCDictValue *VMCDictionary::operator[](const dictKey_t &key)
{
	// Return the existing value if it exists, else insert an empty value
	return &dict[key];
}
const VMCDictValue *VMCDictionary::operator[](const dictKey_t &key) const
{
...
void VMCDictionary::Set(const dictKey_t &key, void *value, int typeId)
{
	dictMap_t::iterator it;
	it = dict.find(key);
	if (it == dict.end())
		it = dict.insert(dictMap_t::value_type(key, VMCDictValue())).first;

	it->second.Set(engine, value, typeId);
}
void VMCDictionary::Set(const dictKey_t &key, const asINT64 &value)
{
	Set(key, const_cast<asINT64*>(&value), asTYPEID_INT64);
}
void VMCDictionary::Set(const dictKey_t &key, const double &value)
{
	Set(key, const_cast<double*>(&value), asTYPEID_DOUBLE);
}
bool VMCDictionary::Get(const dictKey_t &key, void *value, int typeId) const
{
	dictMap_t::const_iterator it;
	it = dict.find(key);
	if (it != dict.end())
...
int VMCDictionary::GetTypeId(const dictKey_t &key) const
{
	dictMap_t::const_iterator it;
	it = dict.find(key);
	if (it != dict.end())
...
bool VMCDictionary::Get(const dictKey_t &key, asINT64 &value) const
{
	return Get(key, &value, asTYPEID_INT64);
}
bool VMCDictionary::Get(const dictKey_t &key, double &value) const
{
	return Get(key, &value, asTYPEID_DOUBLE);
}
bool VMCDictionary::Exists(const dictKey_t &key) const
{
	dictMap_t::const_iterator it;
	it = dict.find(key);
	if (it != dict.end())
...
bool VMCDictionary::IsEmpty() const
{
	if (dict.size() == 0)
		return true;
...
asUINT VMCDictionary::GetSize() const
{
	return asUINT(dict.size());
}
bool VMCDictionary::Delete(const dictKey_t &key)
{
	dictMap_t::iterator it;
	it = dict.find(key);
	if (it != dict.end())
...
VMCArray* VMCDictionary::GetKeys() const
{
	...
	VMCArray *array = VMCArray::Create(ti, asUINT(dict.size()));
	long current = -1;
	dictMap_t::const_iterator it;
	for (it = dict.begin(); it != dict.end(); it++)
	{
...
{
	*(VMCDictionary**)gen->GetAddressOfReturnLocation() = VMCDictionary::Create(gen->GetEngine());
}
void ScriptDictionaryListFactory_Generic(asIScriptGeneric *gen)
{
	asBYTE *buffer = (asBYTE*)gen->GetArgAddress(0);
	*(VMCDictionary**)gen->GetAddressOfReturnLocation() = VMCDictionary::Create(buffer);
}
void ScriptDictionaryAddRef_Generic(asIScriptGeneric *gen)
{
	VMCDictionary *dict = (VMCDictionary*)gen->GetObject();
	dict->AddRef();
}
...
{
	VMCDictionary *dict = (VMCDictionary*)gen->GetObject();
	VMCDictionary *other = *(VMCDictionary**)gen->GetAddressOfArg(0);
	*dict = *other;
	*(VMCDictionary**)gen->GetAddressOfReturnLocation() = dict;
}
...
{
	VMCDictionary *self = (VMCDictionary*)gen->GetObject();
...
{
	VMCDictionary *self = (VMCDictionary*)gen->GetObject();
	*(VMCArray**)gen->GetAddressOfReturnLocation() = self->GetKeys();
}
static void CScriptDictionary_opIndex_Generic(asIScriptGeneric *gen)
{
	VMCDictionary *self = (VMCDictionary*)gen->GetObject();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	*(VMCDictValue**)gen->GetAddressOfReturnLocation() = self->operator[](*key);
}
static void CScriptDictionary_opIndex_const_Generic(asIScriptGeneric *gen)
{
	const VMCDictionary *self = (const VMCDictionary*)gen->GetObject();
	dictKey_t *key = *(dictKey_t**)gen->GetAddressOfArg(0);
	*(const VMCDictValue**)gen->GetAddressOfReturnLocation() = self->operator[](*key);
}

VMCDictValue::VMCDictValue()
{
	m_valueObj = 0;
	m_typeId = 0;
}
VMCDictValue::VMCDictValue(asIScriptEngine *engine, void *value, int typeId)
{
	m_valueObj = 0;
	m_typeId = 0;
...
void VMCDictValue::FreeValue(asIScriptEngine *engine)
{
...
void VMCDictValue::EnumReferences(asIScriptEngine *inEngine)
{
...
void VMCDictValue::Set(asIScriptEngine *engine, void *value, int typeId)
{
	FreeValue(engine);
...
void VMCDictValue::Set(asIScriptEngine *engine, VMCDictValue &value)
{
...
void VMCDictValue::Set(asIScriptEngine *engine, const asINT64 &value)
{
	Set(engine, const_cast<asINT64*>(&value), asTYPEID_INT64);
}
void VMCDictValue::Set(asIScriptEngine *engine, const double &value)
{
	Set(engine, const_cast<double*>(&value), asTYPEID_DOUBLE);
}
bool VMCDictValue::Get(asIScriptEngine *engine, void *value, int typeId) const
{
	// Return the value
	if (typeId & asTYPEID_OBJHANDLE)
	{
...
const void * VMCDictValue::GetAddressOfValue() const
{
...
bool VMCDictValue::Get(asIScriptEngine *engine, asINT64 &value) const
{
	return Get(engine, &value, asTYPEID_INT64);
}
bool VMCDictValue::Get(asIScriptEngine *engine, double &value) const
{
	return Get(engine, &value, asTYPEID_DOUBLE);
}
int VMCDictValue::GetTypeId() const
{
	return m_typeId;
}
...
{
	new(mem) VMCDictValue();
}
static void CScriptDictValue_Destruct(VMCDictValue *obj)
{
	asIScriptContext *ctx = asGetActiveContext();
	if (ctx)
	{
...
static VMCDictValue &CScriptDictValue_opAssign(void *ref, int typeId, VMCDictValue *obj)
{
	asIScriptContext *ctx = asGetActiveContext();
	if (ctx)
	{
...
static VMCDictValue &CScriptDictValue_opAssign(const VMCDictValue &other, VMCDictValue *obj)
{
	asIScriptContext *ctx = asGetActiveContext();
	if (ctx)
	{
		asIScriptEngine *engine = ctx->GetEngine();
		obj->Set(engine, const_cast<VMCDictValue&>(other));
	}

	return *obj;
}
static VMCDictValue &CScriptDictValue_opAssign(double val, VMCDictValue *obj)
{
	return CScriptDictValue_opAssign(&val, asTYPEID_DOUBLE, obj);
}
static VMCDictValue &CScriptDictValue_opAssign(asINT64 val, VMCDictValue *obj)
{
	return CScriptDictValue_opAssign(&val, asTYPEID_INT64, obj);
}
static void CScriptDictValue_opCast(void *ref, int typeId, VMCDictValue *obj)
{
	asIScriptContext *ctx = asGetActiveContext();
	if (ctx)
	{
...
static asINT64 CScriptDictValue_opConvInt(VMCDictValue *obj)
{
	asINT64 value;
	CScriptDictValue_opCast(&value, asTYPEID_INT64, obj);
	return value;
}
static double CScriptDictValue_opConvDouble(VMCDictValue *obj)
{
	double value;
...
{
	VMCDictValue *self = (VMCDictValue*)gen->GetObject();
	double value;
...
{
	VMCDictValue *self = (VMCDictValue*)gen->GetObject();
	*(VMCDictValue**)gen->GetAddressOfReturnLocation() = &CScriptDictValue_opAssign((asINT64)gen->GetArgQWord(0), self);
}
static void CScriptDictValue_opAssign_double_Generic(asIScriptGeneric *gen)
{
	VMCDictValue *self = (VMCDictValue*)gen->GetObject();
	*(VMCDictValue**)gen->GetAddressOfReturnLocation() = &CScriptDictValue_opAssign(gen->GetArgDouble(0), self);
}
static void CScriptDictValue_opAssign_Generic(asIScriptGeneric *gen)
{
	VMCDictValue *self = (VMCDictValue*)gen->GetObject();
	*(VMCDictValue**)gen->GetAddressOfReturnLocation() = &CScriptDictValue_opAssign(gen->GetArgAddress(0), gen->GetArgTypeId(0), self);
}
static void CScriptDictValue_opCopyAssign_Generic(asIScriptGeneric *gen)
{
	VMCDictValue *self = (VMCDictValue*)gen->GetObject();
	*(VMCDictValue**)gen->GetAddressOfReturnLocation() = &CScriptDictValue_opAssign(*reinterpret_cast<VMCDictValue*>(gen->GetArgAddress(0)), self);
}
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectType("dictionaryValue", sizeof(VMCDictValue), asOBJ_VALUE | asOBJ_ASHANDLE | asOBJ_GC | asGetTypeTraits<VMCDictValue>()); assert(r >= 0);
#else
	r = engine->RegisterObjectType("dictionaryValue", sizeof(VMCDictValue), asOBJ_VALUE | asOBJ_ASHANDLE | asOBJ_GC | asOBJ_APP_CLASS_CD); assert(r >= 0);
#endif
	...
	r = engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(VMCDictValue, EnumReferences), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("dictionaryValue", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(VMCDictValue, FreeValue), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(const dictionaryValue &in)", asFUNCTIONPR(CScriptDictValue_opAssign, (const VMCDictValue &, VMCDictValue *), VMCDictValue &), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opHndlAssign(const ?&in)", asFUNCTIONPR(CScriptDictValue_opAssign, (void *, int, VMCDictValue*), VMCDictValue &), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opHndlAssign(const dictionaryValue &in)", asFUNCTIONPR(CScriptDictValue_opAssign, (const VMCDictValue &, VMCDictValue *), VMCDictValue &), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(const ?&in)", asFUNCTIONPR(CScriptDictValue_opAssign, (void *, int, VMCDictValue*), VMCDictValue &), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(double)", asFUNCTIONPR(CScriptDictValue_opAssign, (double, VMCDictValue*), VMCDictValue &), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "dictionaryValue &opAssign(int64)", asFUNCTIONPR(CScriptDictValue_opAssign, (asINT64, VMCDictValue*), VMCDictValue &), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "void opCast(?&out)", asFUNCTIONPR(CScriptDictValue_opCast, (void *, int, VMCDictValue*), void), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "void opConv(?&out)", asFUNCTIONPR(CScriptDictValue_opCast, (void *, int, VMCDictValue*), void), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "int64 opConv()", asFUNCTIONPR(CScriptDictValue_opConvInt, (VMCDictValue*), asINT64), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionaryValue", "double opConv()", asFUNCTIONPR(CScriptDictValue_opConvDouble, (VMCDictValue*), double), asCALL_CDECL_OBJLAST); assert(r >= 0);

	r = engine->RegisterObjectType("dictionary", sizeof(VMCDictionary), asOBJ_REF | asOBJ_GC); assert(r >= 0);
	...
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ADDREF, "void f()", asMETHOD(VMCDictionary, AddRef), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASE, "void f()", asMETHOD(VMCDictionary, Release), asCALL_THISCALL); assert(r >= 0);

	r = engine->RegisterObjectMethod("dictionary", "dictionary &opAssign(const dictionary &in)", asMETHODPR(VMCDictionary, operator=, (const VMCDictionary &), VMCDictionary&), asCALL_THISCALL); assert(r >= 0);

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const ?&in)", asMETHODPR(VMCDictionary, Set, (const dictKey_t&, void*, int), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, ?&out) const", asMETHODPR(VMCDictionary, Get, (const dictKey_t&, void*, int) const, bool), asCALL_THISCALL); assert(r >= 0);

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int64&in)", asMETHODPR(VMCDictionary, Set, (const dictKey_t&, const asINT64&), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int64&out) const", asMETHODPR(VMCDictionary, Get, (const dictKey_t&, asINT64&) const, bool), asCALL_THISCALL); assert(r >= 0);

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const double&in)", asMETHODPR(VMCDictionary, Set, (const dictKey_t&, const double&), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, double&out) const", asMETHODPR(VMCDictionary, Get, (const dictKey_t&, double&) const, bool), asCALL_THISCALL); assert(r >= 0);

	r = engine->RegisterObjectMethod("dictionary", "bool exists(const string &in) const", asMETHOD(VMCDictionary, Exists), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "bool isEmpty() const", asMETHOD(VMCDictionary, IsEmpty), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "uint getSize() const", asMETHOD(VMCDictionary, GetSize), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "bool delete(const string &in)", asMETHOD(VMCDictionary, Delete), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "void deleteAll()", asMETHOD(VMCDictionary, DeleteAll), asCALL_THISCALL); assert(r >= 0);

	r = engine->RegisterObjectMethod("dictionary", "array<string> @getKeys() const", asMETHOD(VMCDictionary, GetKeys), asCALL_THISCALL); assert(r >= 0);

	r = engine->RegisterObjectMethod("dictionary", "dictionaryValue &opIndex(const string &in)", asMETHODPR(VMCDictionary, operator[], (const dictKey_t &), VMCDictValue*), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("dictionary", "const dictionaryValue &opIndex(const string &in) const", asMETHODPR(VMCDictionary, operator[], (const dictKey_t &) const, const VMCDictValue*), asCALL_THISCALL); assert(r >= 0);

	// Register GC behaviours
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_GETREFCOUNT, "int f()", asMETHOD(VMCDictionary, GetRefCount), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_SETGCFLAG, "void f()", asMETHOD(VMCDictionary, SetGCFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_GETGCFLAG, "bool f()", asMETHOD(VMCDictionary, GetGCFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(VMCDictionary, EnumReferences), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(VMCDictionary, ReleaseAllReferences), asCALL_THISCALL); assert(r >= 0);

#if AS_USE_STLNAMES == 1
	// Same as isEmpty
	r = engine->RegisterObjectMethod("dictionary", "bool empty() const", asMETHOD(VMCDictionary, IsEmpty), asCALL_THISCALL); assert(r >= 0);
	// Same as getSize
	r = engine->RegisterObjectMethod("dictionary", "uint size() const", asMETHOD(VMCDictionary, GetSize), asCALL_THISCALL); assert(r >= 0);
	// Same as delete
	r = engine->RegisterObjectMethod("dictionary", "void erase(const string &in)", asMETHOD(VMCDictionary, Delete), asCALL_THISCALL); assert(r >= 0);
	// Same as deleteAll
	r = engine->RegisterObjectMethod("dictionary", "void clear()", asMETHOD(VMCDictionary, DeleteAll), asCALL_THISCALL); assert(r >= 0);
#endif
...
VMCDictionary::CIterator VMCDictionary::begin() const
{
	return CIterator(*this, dict.begin());
}
VMCDictionary::CIterator VMCDictionary::end() const
{
	return CIterator(*this, dict.end());
}
VMCDictionary::CIterator VMCDictionary::find(const dictKey_t &key) const
{
	return CIterator(*this, dict.find(key));
}
VMCDictionary::CIterator::CIterator(const VMCDictionary &dict, dictMap_t::const_iterator it)
	: m_it(it), m_dict(dict)
{}
void VMCDictionary::CIterator::operator++()
{
	++m_it;
}
void VMCDictionary::CIterator::operator++(int)
{
	++m_it;
...
VMCDictionary::CIterator &VMCDictionary::CIterator::operator*()
{
	return *this;
}
bool VMCDictionary::CIterator::operator==(const CIterator &other) const
{
...
const dictKey_t &VMCDictionary::CIterator::GetKey() const
{
	return m_it->first;
}
int VMCDictionary::CIterator::GetTypeId() const
{
	return m_it->second.m_typeId;
}
bool VMCDictionary::CIterator::GetValue(asINT64 &value) const
{
	return m_it->second.Get(m_dict.engine, &value, asTYPEID_INT64);
}
bool VMCDictionary::CIterator::GetValue(double &value) const
{
	return m_it->second.Get(m_dict.engine, &value, asTYPEID_DOUBLE);
}
bool VMCDictionary::CIterator::GetValue(void *value, int typeId) const
{
	return m_it->second.Get(m_dict.engine, value, typeId);
}
const void *VMCDictionary::CIterator::GetAddressOfValue() const
{
