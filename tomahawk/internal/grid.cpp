...
void VMCGrid::SetMemoryFunctions(asALLOCFUNC_t allocFunc, asFREEFUNC_t freeFunc)
{
	userAlloc = allocFunc;
	userFree = freeFunc;
}
...
VMCGrid *VMCGrid::Create(asITypeInfo *ti)
{
	return VMCGrid::Create(ti, 0, 0);
}
VMCGrid *VMCGrid::Create(asITypeInfo *ti, asUINT w, asUINT h)
{
	// Allocate the memory
	void *mem = userAlloc(sizeof(VMCGrid));
	if (mem == 0)
	{
...
VMCGrid *VMCGrid::Create(asITypeInfo *ti, void *initList)
{
	// Allocate the memory
	void *mem = userAlloc(sizeof(VMCGrid));
	if (mem == 0)
...
VMCGrid *VMCGrid::Create(asITypeInfo *ti, asUINT w, asUINT h, void *defVal)
{
	// Allocate the memory
	void *mem = userAlloc(sizeof(VMCGrid));
	if (mem == 0)
...
static void RegisterScriptGrid_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_FACTORY, "grid<T>@ f(int&in)", asFUNCTIONPR(VMCGrid::Create, (asITypeInfo*), VMCGrid*), asCALL_CDECL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_FACTORY, "grid<T>@ f(int&in, uint, uint)", asFUNCTIONPR(VMCGrid::Create, (asITypeInfo*, asUINT, asUINT), VMCGrid*), asCALL_CDECL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_FACTORY, "grid<T>@ f(int&in, uint, uint, const T &in)", asFUNCTIONPR(VMCGrid::Create, (asITypeInfo*, asUINT, asUINT, void *), VMCGrid*), asCALL_CDECL); assert(r >= 0);

	// Register the factory that will be used for initialization lists
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_LIST_FACTORY, "grid<T>@ f(int&in type, int&in list) {repeat {repeat_same T}}", asFUNCTIONPR(VMCGrid::Create, (asITypeInfo*, void*), VMCGrid*), asCALL_CDECL); assert(r >= 0);

	// The memory management methods
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_ADDREF, "void f()", asMETHOD(VMCGrid, AddRef), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_RELEASE, "void f()", asMETHOD(VMCGrid, Release), asCALL_THISCALL); assert(r >= 0);

	// The index operator returns the template subtype
	r = engine->RegisterObjectMethod("grid<T>", "T &opIndex(uint, uint)", asMETHODPR(VMCGrid, At, (asUINT, asUINT), void*), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("grid<T>", "const T &opIndex(uint, uint) const", asMETHODPR(VMCGrid, At, (asUINT, asUINT) const, const void*), asCALL_THISCALL); assert(r >= 0);

	// Other methods
	r = engine->RegisterObjectMethod("grid<T>", "void resize(uint width, uint height)", asMETHODPR(VMCGrid, Resize, (asUINT, asUINT), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("grid<T>", "uint width() const", asMETHOD(VMCGrid, GetWidth), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("grid<T>", "uint height() const", asMETHOD(VMCGrid, GetHeight), asCALL_THISCALL); assert(r >= 0);

	// Register GC behaviours in case the array needs to be garbage collected
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_GETREFCOUNT, "int f()", asMETHOD(VMCGrid, GetRefCount), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_SETGCFLAG, "void f()", asMETHOD(VMCGrid, SetFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_GETGCFLAG, "bool f()", asMETHOD(VMCGrid, GetFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(VMCGrid, EnumReferences), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("grid<T>", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(VMCGrid, ReleaseAllHandles), asCALL_THISCALL); assert(r >= 0);
}

VMCGrid::VMCGrid(asITypeInfo *ti, void *buf)
{
	refCount = 1;
	gcFlag = false;
	objType = ti;
...
VMCGrid::VMCGrid(asUINT width, asUINT height, asITypeInfo *ti)
{
	refCount = 1;
	gcFlag = false;
	objType = ti;
...
void VMCGrid::Resize(asUINT width, asUINT height)
{
...
VMCGrid::VMCGrid(asUINT width, asUINT height, void *defVal, asITypeInfo *ti)
{
	refCount = 1;
	gcFlag = false;
	objType = ti;
...
void VMCGrid::SetValue(asUINT x, asUINT y, void *value)
{
	SetValue(buffer, x, y, value);
}
void VMCGrid::SetValue(SGridBuffer *buf, asUINT x, asUINT y, void *value)
{
...
asUINT VMCGrid::GetHeight() const
{
	if (buffer)
...
bool VMCGrid::CheckMaxSize(asUINT width, asUINT height)
{
...
asITypeInfo *VMCGrid::GetGridObjectType() const
{
	return objType;
}
...
void *VMCGrid::At(asUINT x, asUINT y)
{
	return At(buffer, x, y);
}
void *VMCGrid::At(SGridBuffer *buf, asUINT x, asUINT y)
{
...
const void *VMCGrid::At(asUINT x, asUINT y) const
{
	return const_cast<VMCGrid*>(this)->At(const_cast<SGridBuffer*>(buffer), x, y);
}
void VMCGrid::CreateBuffer(SGridBuffer **buf, asUINT w, asUINT h)
{
...
void VMCGrid::DeleteBuffer(SGridBuffer *buf)
{
	assert(buf);
...
void VMCGrid::Construct(SGridBuffer *buf)
{
	assert(buf);
...
void VMCGrid::Destruct(SGridBuffer *buf)
{
	assert(buf);
...
void VMCGrid::EnumReferences(asIScriptEngine *engine)
{
	if (buffer == 0) return;
...
void VMCGrid::ReleaseAllHandles(asIScriptEngine*)
{
	if (buffer == 0) return;
...
void VMCGrid::Release() const
{
	...
	if (asAtomicDec(refCount) == 0)
	{
		...
		this->~VMCGrid();
		userFree(const_cast<VMCGrid*>(this));
	}
}
