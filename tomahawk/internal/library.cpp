...
int VM_CompareEquality(asIScriptEngine *engine, void *lobj, void *robj, int typeId, bool &result)
{
	...
	else
	{
		...
		int relation;
		retval = VM_CompareRelation(engine, lobj, robj, typeId, relation);
		if (retval >= 0)
...
int VM_ExecuteString(asIScriptEngine *engine, const char *code, asIScriptModule *mod, asIScriptContext *ctx)
{
	return VM_ExecuteString(engine, code, 0, asTYPEID_VOID, mod, ctx);
}
int VM_ExecuteString(asIScriptEngine *engine, const char *code, void *ref, int refTypeId, asIScriptModule *mod, asIScriptContext *ctx)
{
...
int VM_WriteConfigToFile(asIScriptEngine *engine, const char *filename)
{
	...
	return VM_WriteConfigToStream(engine, strm);
}
int VM_WriteConfigToStream(asIScriptEngine *engine, ostream &strm)
{
...
int VM_ConfigEngineFromStream(asIScriptEngine *engine, istream &strm, const char *configFile, asIStringFactory *stringFactory)
{
	int r;
...
string VM_GetExceptionInfo(asIScriptContext *ctx, bool showStack)
{
