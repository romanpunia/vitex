...
namespace Tomahawk
{
	namespace Network
	{
		...
		int Socket::Accept(Socket* Connection, Address* Output)
		{
			...
			sockaddr Address; socket_size_t Length = sizeof(sockaddr);
		...
		int Socket::GetPort()
		{
			struct sockaddr_storage Address; socket_size_t Size = sizeof(Address);
		...
		std::string Socket::LocalIpAddress()
		{
			...
			return Rest::Form("%i.%i.%i.%i",
                    ...
                    (int)((struct in_addr *)(Host->h_addr))->S_un.S_un_b.s_b4).R();
#else
			...
			if (*Events & SocketEvent_Write && !(*Events & SocketEvent_Close))
			{
				if (Fd->Output != nullptr)
				{
					...
					while (Result != -1)
					{
						...
						WriteEvent* Event = Fd->Output; Int64 Offset = 0;
