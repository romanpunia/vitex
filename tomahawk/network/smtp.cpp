	...
	namespace Network
	{
		namespace SMTP
		{
			...
			bool Client::Authorize(const ReplyCallback& Callback)
			{
				...
				else if (CanRequest("CRAM-MD5"))
				{
					return SendRequest(334, "AUTH CRAM-MD5\r\n", [this, Callback]()
					{
						...
						memset(IPad, 0, 64); memset(OPad, 0, 64);
			...
			bool Client::SendAttachment()
			{
				...
				const char* Name = It.Path.c_str(); UInt64 Id = strlen(Name) - 1;
			...
			{		
				Attachment& It = Request.Attachments.at(Request.Attachments.size() - Pending); char Data[8192];
