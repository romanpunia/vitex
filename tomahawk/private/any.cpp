...
static void ScriptAnyFactory_Generic(asIScriptGeneric *gen)
{
	...
	*(VMCAny**)gen->GetAddressOfReturnLocation() = new VMCAny(engine);
}
static void ScriptAnyFactory2_Generic(asIScriptGeneric *gen)
{
	...
	*(VMCAny**)gen->GetAddressOfReturnLocation() = new VMCAny(ref, refType, engine);
}
static VMCAny &ScriptAnyAssignment(VMCAny *other, VMCAny *self)
{
	return *self = *other;
}
static void ScriptAnyAssignment_Generic(asIScriptGeneric *gen)
{
	VMCAny *other = (VMCAny*)gen->GetArgObject(0);
	VMCAny *self = (VMCAny*)gen->GetObject();
...
{
	int r;
	r = engine->RegisterObjectType("any", sizeof(VMCAny), asOBJ_REF | asOBJ_GC); assert(r >= 0);
	...
	r = engine->RegisterObjectBehaviour("any", asBEHAVE_ADDREF, "void f()", asMETHOD(VMCAny, AddRef), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("any", asBEHAVE_RELEASE, "void f()", asMETHOD(VMCAny, Release), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("any", "any &opAssign(any&in)", asFUNCTION(ScriptAnyAssignment), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("any", "void store(?&in)", asMETHODPR(VMCAny, Store, (void*, int), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("any", "void store(const int64&in)", asMETHODPR(VMCAny, Store, (asINT64&), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("any", "void store(const double&in)", asMETHODPR(VMCAny, Store, (double&), void), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("any", "bool retrieve(?&out)", asMETHODPR(VMCAny, Retrieve, (void*, int) const, bool), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("any", "bool retrieve(int64&out)", asMETHODPR(VMCAny, Retrieve, (asINT64&) const, bool), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("any", "bool retrieve(double&out)", asMETHODPR(VMCAny, Retrieve, (double&) const, bool), asCALL_THISCALL); assert(r >= 0);

	// Register GC behaviours
	r = engine->RegisterObjectBehaviour("any", asBEHAVE_GETREFCOUNT, "int f()", asMETHOD(VMCAny, GetRefCount), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("any", asBEHAVE_SETGCFLAG, "void f()", asMETHOD(VMCAny, SetFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("any", asBEHAVE_GETGCFLAG, "bool f()", asMETHOD(VMCAny, GetFlag), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("any", asBEHAVE_ENUMREFS, "void f(int&in)", asMETHOD(VMCAny, EnumReferences), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectBehaviour("any", asBEHAVE_RELEASEREFS, "void f(int&in)", asMETHOD(VMCAny, ReleaseAllHandles), asCALL_THISCALL); assert(r >= 0);
}
...
VMCAny::VMCAny(asIScriptEngine *engine)
{
	this->engine = engine;
	refCount = 1;
	gcFlag = false;
...
VMCAny::VMCAny(void *ref, int refTypeId, asIScriptEngine *engine)
{
	this->engine = engine;
	refCount = 1;
	gcFlag = false;
...
bool VMCAny::Retrieve(asINT64 &outValue) const
{
...
void VMCAny::EnumReferences(asIScriptEngine *inEngine)
{
...
void VMCAny::ReleaseAllHandles(asIScriptEngine*)
{
	FreeObject();
}
